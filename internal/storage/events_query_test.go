...
func TestBuildEventsQuery(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(tmpDir, "events_0_100.parquet"), []byte{}, 0644); err != nil {
		t.Fatalf("failed to create parquet file: %v", err)
	}
	if err := os.WriteFile(filepath.Join(tmpDir, "events_200_300.parquet"), []byte{}, 0644); err != nil {
		t.Fatalf("failed to create parquet file: %v", err)
	}

	s := &Storage{
		dataDir:     tmpDir,
		jsonlWriter: NewJSONLWriter(tmpDir),
	}
	...
	if !strings.Contains(final, "events_0_100.parquet") {
