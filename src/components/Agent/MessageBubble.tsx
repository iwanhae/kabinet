...
export const MessageBubble: React.FC<MessageBubbleProps> = ({ message }) => {
    ...
    if (isSystem) {
        if (isQueryResult) {
            ...
            return (
                <Accordion variant="outlined" sx={{ mb: 2, bgcolor: 'background.paper' }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        {parsedPlan.thought && (
                            <Box sx={{ display: 'flex', gap: 1.5, color: 'text.secondary' }}>
                                <FormatQuoteIcon fontSize="small" sx={{ mt: -0.2, opacity: 0.5 }} />
                        ...
                        {parsedPlan.hypothesis && (
                            <Box sx={{
                                ...
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
                                    ...
                                    <Typography variant="caption" sx={{ fontWeight: 700, letterSpacing: 0.5, color: 'warning.main' }}>
                                        HYPOTHESIS
                                    </Typography>
                                </Box>
